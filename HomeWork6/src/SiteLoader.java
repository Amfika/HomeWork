...
public abstract class SiteLoader {
    ...
    public enum Currency{
        ...
        private String id; //переменная с id

        Currency(String id) { //метод 1
            this.id = id;
        }

        public String getId(){ //метод 2
            return this.id;
        }
    }
    ...
    protected final double load(String urlToSite, SiteLoader.Currency currencyName){
        ...
        return handle(content.toString(), currencyName); //вызов метода handle с 2 параметрами String и Сurrency и возвращает результат своей работы
    }
